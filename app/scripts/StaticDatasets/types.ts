...
type MapDataDatasetsSource = MapDataDatasetsSourceBase & {
  /** @desc MapDatasetCategory.key for the region sidebar; labels and order live in DB. */
...
/**
 * File-level metadata — one value per dataset file, not per layer-config "Ansicht". Stored on
 * `MapDatasetUpload` columns and at the `meta.ts` root.
 */
export type MetaDataFileLevelMetadata = {
  /** @desc Entity that has to be named as creator (c) of the data on the map and at the dataset */
  attributionHtml: string
  /** @desc A link to the source or a description of how the data was created */
  dataSourceMarkdown?: string
  /** @desc Free-text dataset freshness label (e.g. `2024-11-13`) */
...
export type MetaDataUser = MetaDataFileLevelMetadata &
  (
    | {
        regions: string[]
        public: boolean
...
export type MetaDataSystemLayer = MetaDataFileLevelMetadata & {
  regions: string[]
  public: true
